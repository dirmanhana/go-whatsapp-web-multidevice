...
var (
	...
	ErrCannotDeleteSelf    = ForbiddenError("an admin cannot delete their own account")
	ErrCannotDemoteSelf    = ForbiddenError("an admin cannot remove their own admin access")
	ErrUserOwnsDevices     = ForbiddenError("user still owns devices; remove the devices before deleting the account")
	ErrCurrentPasswordBad  = InvalidCredentialsError("current password is incorrect")
